...
La patch ACSD-45817 risolve il problema relativo alla presenza di un GraphQL `products` la mutazione per un archivio specifico restituisce tutte le varianti configurabili, incluse quelle non assegnate all’archivio richiesto. Questa patch è disponibile quando [Strumento Patch di qualità (QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 1.1.18. L’ID della patch è ACSD-45817. Il problema è stato risolto in Adobe Commerce 2.4.4.
...
A GRAPHQL `products` la mutazione per un archivio specifico restituisce tutte le varianti configurabili, incluse quelle non assegnate all’archivio richiesto.
...
1. Vai a **Vetrina**, passare al secondo sito Web e aprire il prodotto configurabile.
1. Assicurati di visualizzare solo un’opzione figlio: &quot;configurable-a&quot;.
1. Eseguire una query GraphQL tramite `POST: /graphql` endpoint, e `Headers: "store" = "new"`
