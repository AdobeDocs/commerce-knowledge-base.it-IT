...
In questo esempio, creiamo un nuovo modulo Adobe Commerce con una patch di dati che viene applicata al momento dell’installazione o del processo di aggiornamento di Adobe Commerce e aggiunge ad Adobe Commerce un Paese astratto con il codice paese XX. Il [Directory di Adobe Commerce](https://developer.adobe.com/commerce/php/module-reference/module-directory/) crea un elenco dei paesi iniziale e utilizza le patch di configurazione per aggiungere territori all&#39;elenco. Questo articolo spiega come creare un nuovo modulo che aggiungerà un nuovo paese all’elenco. È possibile consultare il codice del modulo Adobe Commerce Directory esistente come riferimento. Questo perché il modulo di esempio seguente continua il processo del modulo Directory di creazione di un elenco di paesi e aree geografiche e riutilizza parti del codice delle patch di installazione del modulo di Adobe Commerce Directory.
...
* [Panoramica del modulo](https://devdocs.magento.com/guides/v2.4/architecture/archi_perspectives/components/modules/mod_intro.html)
* [Crea un nuovo modulo](https://devdocs.magento.com/videos/fundamentals/create-a-new-module/)
* [File di configurazione del modulo](https://devdocs.magento.com/guides/v2.4/config-guide/config/config-files.html)
...
<pre><ExtraCountries>
 ...
 | | | <Patch>
 | | |   <Data>
 | | | AddDataForAbstractCountry.php | compositore.json registration.php</pre>
...
* `allow` - Per aggiungere il nuovo paese all’elenco &quot;Paesi consentiti&quot; per impostazione predefinita, aggiungi il nuovo codice paese alla fine del `allow` contenuti dei tag. I codici paese sono separati da virgola. Tieni presente che questo tag sovrascriverà i dati provenienti da `Directory` file di configurazione del modulo *(Directory/etc/config.xml)* `allow` questo è il motivo per cui ripetiamo tutti i codici qui e aggiungiamo quello nuovo.
* `optional_zip_countries` - Se il codice postale del nuovo paese aggiunto deve essere facoltativo, aggiungi il codice del paese alla fine del contenuto della `optional_zip_countries` tag. I codici paese sono separati da virgola. Tieni presente che questo tag sovrascriverà i dati provenienti da `Directory` file di configurazione del modulo *(Directory/etc/config.xml)* `optional_zip_countries` questo è il motivo per cui ripetiamo tutti i codici qui e aggiungiamo quello nuovo.
* `eu_countries` - Se il paese appena aggiunto deve far parte per impostazione predefinita dell’elenco dei paesi dell’Unione europea, aggiungi il codice del paese alla fine del contenuto della `eu_countries` tag. I codici paese sono separati da virgola. Tieni presente che questo tag sovrascriverà i dati provenienti da `Store` file di configurazione del modulo *(\_Store/etc/config.xml\_)* `eu_countries` questo è il motivo per cui ripetiamo tutti i codici qui e aggiungiamo quello nuovo.
...
Per ulteriori informazioni sui file di configurazione del modulo, vedi [Guida per gli sviluppatori PHP > Definisci i file di configurazione](https://devdocs.magento.com/guides/v2.4/extension-dev-guide/build/required-configuration-files.html) nella documentazione per gli sviluppatori.

Queste modifiche sono facoltative e influiranno solo sull’appartenenza predefinita del nuovo paese agli elenchi &quot;Paesi consentiti&quot;, &quot;CAP è facoltativo per&quot; e &quot;Paesi dell’Unione europea&quot;. Se questo file viene saltato dalla struttura del modulo, verrà comunque aggiunto un nuovo paese, ma dovrà essere configurato manualmente in **Amministratore** > **Negozi** > *Impostazioni* > **Configurazione** > **Generale** > **Opzioni paese** impostazioni.

### ExtraCountries/etc/di.xml

Il `di.xml` file configura le dipendenze inserite dal gestore oggetti. Consulta <a>Guida per gli sviluppatori PHP > Il di.xml</a> nella documentazione per gli sviluppatori di per ulteriori informazioni su `di.xml`.

Nel nostro esempio, è necessario registrare una `_TranslatedListsPlugin_` che tradurrà i codici paese appena introdotti in nomi paese completi, se i codici non sono presenti nei dati di localizzazione della Zend Locale Library.
...
Consulta [Gestione delle dipendenze dei moduli](https://devdocs.magento.com/guides/v2.4/architecture/archi_perspectives/components/modules/mod_depend.html#managing-module-dependencies) nella documentazione per gli sviluppatori di per ulteriori informazioni sulle dipendenze dei moduli.
...
In `aroundGetCountryTranslation()` metodo del plug-in dobbiamo tradurre un codice paese in un nome paese completo. Questo passaggio è obbligatorio per i paesi a cui non è associato un nome completo con un nuovo codice paese nella libreria locale di Zend.
...
Consulta [Sviluppare patch di dati e schemi](https://devdocs.magento.com/guides/v2.4/extension-dev-guide/declarative-schema/data-patches.html) nella documentazione per gli sviluppatori di per ulteriori informazioni sulle patch di dati.

Nell’esempio seguente, puoi vedere che il `$data` array del metodo `apply()` contiene i codici paese, ISO2 e ISO3 per il nuovo paese e questi dati vengono inseriti nel database.
...
Questo è un esempio del file registration.php. Per ulteriori informazioni sulla registrazione dei moduli, consulta [Guida per gli sviluppatori PHP > Registrazione del componente](https://devdocs.magento.com/guides/v2.4/extension-dev-guide/build/component-registration.html) nella documentazione per gli sviluppatori.
...
Per ulteriori informazioni su compositore.json, consulta [Guida per gli sviluppatori PHP > Il file compositore.json](https://devdocs.magento.com/guides/v2.4/extension-dev-guide/build/composer-integration.html) nella documentazione per gli sviluppatori.
...
Per informazioni su come installare il modulo, consulta [Posizioni del modulo](https://devdocs.magento.com/guides/v2.4/architecture/archi_perspectives/components/modules/mod_intro.html#module-locations) nella documentazione per gli sviluppatori.
