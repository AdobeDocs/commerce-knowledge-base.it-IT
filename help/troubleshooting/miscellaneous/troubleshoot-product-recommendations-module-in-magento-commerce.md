...
1. Verificare che [[!UICONTROL indexers]](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/manage-indexers) (nella documentazione per gli sviluppatori) sia in esecuzione e che `Product Feed` [!UICONTROL indexer] sia impostato su `Update by Schedule`.
1. Verificare che i moduli siano *abilitati*. Il metapackage `saas-export` installa i seguenti moduli, che devono essere tutti *abilitati*:
